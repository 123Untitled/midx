	...
	struct debug final {
		...
		private:
			...
			auto _visit(const mx::usz index) -> void {
				self d{_tree, _indent + 2U};
				d._switch(_tree.remap(index));
			}


			debug(const as::tree& t, const mx::usz ind = 0U) noexcept
			: _tree{t}, _indent{ind} {
			}
		...
		public:
			...
			static auto run(const as::tree& t, const as::node& n) -> void {
				as::debug{t}._switch(n);
			}
